...
pub trait Rpc {
    ...
    fn send_transaction(&self, params: Transaction) -> Result<String>;
...
impl Rpc for RpcImpl {
    ...
    // 接收客户端传来的Transaction,并将Transaction 中的from字段和同字段合并，返回给客户端
    fn send_transaction(&self, params: Transaction) -> Result<String> {
        // 将结构体参数解析成json字符串
        let res = serde_json::to_string(&params);
        let s = match res {
            Ok(i) => i,
            Err(_e) => "error".to_string()
        };
        println!("{}",s);
        // 合并结构体中的 from 字段和 to 字段
        let result = params.from + &params.to;
        println!("{}", result);
    ...
    thread::spawn(move || loop {
        {
            ...
            for sink in subscribers.values() {
                let _ = sink.notify(Ok("hello_world".into())).wait();
            }
        }
